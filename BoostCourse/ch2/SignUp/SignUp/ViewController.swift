...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func touchUpSignInButton(_ sender: UIButton) {
        ...
        if loginSuccess == true {
            ...
            // 로그인 성공 시 다음 화면으로
            let homeStoryboard: UIStoryboard = UIStoryboard(name: "Tab", bundle: nil)
            let home = homeStoryboard.instantiateViewController(withIdentifier: "MusicVC") as! MusicViewController
...
/*
            // 탭바 추가
            let tabStoryboard: UIStoryboard = UIStoryboard(name: "Tab", bundle: nil)
            guard let tb = tabStoryboard.instantiateViewController(withIdentifier: "MusicVC") as? UITabBarController else { return }

            guard let first = self.storyboard?.instantiateViewController(withIdentifier: "MusicVC") as? MusicViewController else { return }
            guard let second = self.storyboard?.instantiateViewController(withIdentifier: "SecondTab") as? SecondTabViewController  else { return }
            guard let third = self.storyboard?.instantiateViewController(withIdentifier: "ThirdTab") as? ThirdTabViewController else { return }
            guard let last = self.storyboard?.instantiateViewController(withIdentifier: "LastTab") as? LastTabViewController else { return }

            tb.setViewControllers([first, second, third, last], animated: true)

            first.tabBarItem = UITabBarItem(tabBarSystemItem: UITabBarItem.SystemItem.featured, tag: 0)
            second.tabBarItem = UITabBarItem(tabBarSystemItem: UITabBarItem.SystemItem.favorites, tag: 1)
            third.tabBarItem = UITabBarItem(tabBarSystemItem: UITabBarItem.SystemItem.contacts, tag: 2)
            last.tabBarItem = UITabBarItem(tabBarSystemItem: UITabBarItem.SystemItem.more, tag: 3)
...
*/
    ...
    override func viewWillAppear(_ animated: Bool) {
        self.idField.text = UserModel.shared.id
        self.pwField.text = UserModel.shared.pw
    }
